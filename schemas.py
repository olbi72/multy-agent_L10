...
class LocalCritiqueResult(BaseModel):
    verdict: Literal["APPROVE", "REVISE"]
    is_grounded_in_kb: bool = Field(
        description="Are the findings grounded in the local knowledge base documents?"
    )
    covers_main_points: bool = Field(
        description="Do the findings cover the main ideas from the local documents?"
    )
    is_well_structured: bool = Field(
        description="Are the findings logically organized and ready for a report?"
    )
    strengths: list[str] = Field(
        description="What is good about the findings"
    )
    gaps: list[str] = Field(
        description="What is missing, unsupported, or drifting away from the local corpus"
    )
    revision_requests: list[str] = Field(
        description="Specific things to fix if verdict is REVISE"
    )